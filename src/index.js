...
export function analyze(source, options = {}) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (!current) {
      bindings.set(name, {
        ...
        origin: { file, ...locationFor(raw, name, lineNumber) },
        assignments: [{ file, ...locationFor(raw, expression, lineNumber), inferred: inferred ?? 'unknown' }],
      });
      continue;
    }
    ...
    if (current.type !== 'unknown' && inferred && current.type !== inferred) {
      diagnostics.push({
        code: 'NOVA-T1001',
        severity: 'error',
        message: `Conflicting inferred types for '${name}': ${current.type} then ${inferred}`,
        file,
        ...locationFor(raw, expression, lineNumber),
        origin: current.origin,
        notes: [`'${name}' was first inferred as ${current.type} at ${current.origin.file}:${current.origin.line}:${current.origin.column}`],
      });
    } else if (current.type === 'unknown' && inferred) {
  ...
  return {
    ...
    bindings: Object.fromEntries([...bindings.entries()].map(([name, value]) => [name, value])),
    diagnostics,
  };
}

export function assertValid(source, options = {}) {
  const result = analyze(source, options);
  if (!result.ok) throw new NovaDiagnosticError(result.diagnostics);
  return result;
}
